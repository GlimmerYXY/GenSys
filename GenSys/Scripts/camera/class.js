...
function DeviceInfo() {
    ...
    function Init(p) {
        ...
        p.SearchByMon = function (RecordType, RecordTime) {
            return (gOcx.SearchByMon(RecordType, RecordTime))
        };
        ...
        p.SetPreviewRecordStatus = function(chn,type,bshow,r,g,b){
        	return (gOcx.SetPreviewRecordStatus(chn,type,bshow,r,g,b));
        }
	p.openSafariByUrl = function (url) {
		...
		p.paramExportEXCEL = function(path,filename,row,col,file){
			return(gOcx.paramExportEXCEL(path,filename,row,col,file));
		}
